...
if os.path.exists(f"temp_size.txt"):
    os.remove(f"temp_size.txt")
with open(f"temp_size.txt", "a") as f:
...
ax1.text(0.02, 0.95, f'$T_{{\\text{{ref}}}} = {temp_ref:.3f}$', transform=ax1.transAxes, fontsize=10, verticalalignment='top')
ax1.text(0.02, 0.85, f'$T_{{\\text{{mean}}}} = {temp_mean:.3f} \pm {temp_err:.3f}$', transform=ax1.transAxes, fontsize=10, verticalalignment='top')
