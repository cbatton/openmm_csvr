...
# PrettyPrintableIntegrator and ThermostatedIntegrator are taken from openmmtools
...
class CSVRIntegrator(ThermostatedIntegrator):
    ...
    """CSVR thermostat using either the middle or the velocity Verlet scheme
    ...
    def thermostat_step(self):
        ...
        Following the implementation of https://arxiv.org/pdf/0901.0779.pdf
        The implementation is inspired mostly by what is used in LAMMPS per
        https://github.com/lammps/lammps/blob/stable/src/EXTRA-FIX/fix_temp_csvr.h
                mixed with the code implementation in HOOMD-blue per
                https://github.com/glotzerlab/hoomd-blue/blob/trunk-patch/hoomd/md/Thermostat.h
        """
    ...
    def sum_noises(self):
        ...
        Calculate the same of self.dof-1 gaussian random numbers.
                Use a gamma distribution to do so efficiently.
        """
